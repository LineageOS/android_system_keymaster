...
std::vector<uint8_t> make_test_mac_key() {
...
void AndroidKeymaster::GenerateCsr(const GenerateCsrRequest& request,
    ...
    auto pubKeysToSign =
        validateAndExtractPubkeys(request.test_mode, request.num_keys, request.keys_to_sign_array,
                                  request.test_mode ? make_test_mac_key() : rem_prov_ctx->macKey_);
    ...
    auto pubKeysToSignMac =
        generateCoseMac0Mac(ephemeral_mac_key, std::vector<uint8_t>{}, *pubKeysToSign);
    if (!pubKeysToSignMac) {
